...
def Slice_Raw_Events_onTemporal(deltatime, seq):
    ...
    for i in range (0,round(max(tdata1)/deltatime)):
        ...
        print("tyoe!!!!!!!!!!!!!!!!!!!!!!!!!", (seq_data1).shape)
        ind = np.arange(seq_data1.shape[0])
        np.random.shuffle(ind)
        seq_data1_suffled= seq_data1[ind]   
        print("seq_data1_suffled", seq_data1_suffled.shape)
        print(round(len(seq_data1)/3))
        numberv=round(len(seq_data1)/3)
        seq_data1_suffled_totkae=seq_data1_suffled[-numberv:]
        print("seq_data1_suffled_totkae", seq_data1_suffled_totkae.shape)
        seq_data.append((seq_data1_suffled_totkae))
    return seq_data
...
def Process_creat_Graphs(seq_data, alpha):
    ...
    # if len(seq_data[0])>0:
    tdata=alpha*(seq_data [:, 2]-seq_data [0, 2])*(1/((1/3)*0.01))
    # if len(seq_data[0])==0:
...
Path="/l/proj/kuin0013/Yusra_Sept/Evimo2_trainingdataset_typeD_10msOver_xyOver3/EVIMO2_GL_training/"
